...
def markdown_to_plaintext(markdown_file):
    with open(markdown_file, encoding="UTF-8", errors="ignore") as f:
...
def export_aggregated_license(export_path, sources_path, license_files, header, include_license_texts=True):
    ...
    with open(export_path, 'w',encoding="UTF-8", errors="ignore") as outfile:
        ...
        if include_license_texts:
            ...
            for fname in license_files:
                with open(os.path.join(path_utils.root_dir, fname),encoding="UTF-8", errors="ignore") as infile:
